...
class MountainCarEndpointDataset(Dataset):
    def __init__(self, data_file: str,
        ...
        if dataset_dir is None:
            dataset_dir = "/common/users/shared/pracsys/genMoPlan/data_trajectories/mountain_car_power_0p001"
...
class MountainCarEndpointDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage is None:
            self.train_dataset = MountainCarEndpointDataset(self.data_file, dataset_dir=self.dataset_dir)
            self.val_dataset = MountainCarEndpointDataset(self.validation_file, dataset_dir=self.dataset_dir)

        if stage == "test" or stage is None:
            self.test_dataset = MountainCarEndpointDataset(self.test_file, dataset_dir=self.dataset_dir)
