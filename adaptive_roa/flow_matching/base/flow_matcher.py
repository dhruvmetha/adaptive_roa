...
class BaseFlowMatcher(pl.LightningModule, ABC):
    ...
    def compute_evaluation_metrics(self,
        ...
        # Get dimension names
        mae_per_dim_names = [self._get_dimension_name(i) for i in range(self.system.state_dim)]
    ...
    def evaluate_on_dataset(self,
        ...
        return {
            ...
            'mae_per_dim_names': mae_per_dim_names,
            'mae_per_dim_dict': {name: mae_per_dim[i].item() for i, name in enumerate(mae_per_dim_names)},
    ...
    def compute_manifold_distance_per_component(self,
                                                 predicted_endpoints: torch.Tensor,
                                                 true_endpoints: torch.Tensor) -> torch.Tensor:
        """
    ...
    def compute_grouped_euclidean_distances(self,
                                            predicted_endpoints: torch.Tensor,
                                            true_endpoints: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
    ...
    def get_manifold_component_names(self) -> list:
        ...
        num_components = sum(
            1 if hasattr(m, 'dist') and not hasattr(m, 'state_dims') else getattr(m, 'state_dims', [1])[0] if hasattr(m, 'state_dims') else 1
            for m in self.manifold.manifolds
        )
        return [f"component_{i}" for i in range(num_components)]

    def evaluate_with_manifold_metrics(self,
                                       dataloader,
                                       num_steps: int = 100,
                                       device: Optional[torch.device] = None,
                                       attractor_radius: float = 0.05) -> Dict[str, Any]:
        """
        ...
        in_attractor = self.system.is_in_attractor(all_predicted, radius=attractor_radius)
        ...
        # ===================================================================
        full_stats = self._compute_subset_stats(manifold_distances, component_names, "full")
        ...
        if n_certain > 0:
            ...
            certain_stats = self._compute_subset_stats(certain_distances, component_names, "certain")
        else:
        ...
        if n_uncertain > 0:
            ...
            uncertain_stats = self._compute_subset_stats(uncertain_distances, component_names, "uncertain")
        else:
        ...
        if n_certain > 0:
            ...
            aggregated_stats['aggregated_certain_var'] = certain_agg.var().item() if n_certain > 1 else 0.0
            aggregated_stats['aggregated_certain_std'] = certain_agg.std().item() if n_certain > 1 else 0.0
        else:
        ...
        if n_uncertain > 0:
            ...
            aggregated_stats['aggregated_uncertain_median'] = uncertain_agg.median().item()
            aggregated_stats['aggregated_uncertain_var'] = uncertain_agg.var().item() if n_uncertain > 1 else 0.0
            aggregated_stats['aggregated_uncertain_std'] = uncertain_agg.std().item() if n_uncertain > 1 else 0.0
        else:
        ...
        # ===================================================================
        grouped_distances = self.compute_grouped_euclidean_distances(all_predicted, all_true)
        ...
        if n_certain > 0:
            ...
            rmse_stats['certain_var'] = certain_rmse.var().item() if n_certain > 1 else 0.0
            rmse_stats['certain_std'] = certain_rmse.std().item() if n_certain > 1 else 0.0
        else:
        ...
        if n_uncertain > 0:
            ...
            rmse_stats['uncertain_var'] = uncertain_rmse.var().item() if n_uncertain > 1 else 0.0
            rmse_stats['uncertain_std'] = uncertain_rmse.std().item() if n_uncertain > 1 else 0.0
        else:
    ...
    def _compute_subset_stats(self,
                              distances: torch.Tensor,
                              component_names: list,
                              subset_name: str) -> Dict[str, Any]:
        """
        ...
        stats['mae_per_component'] = [per_component[name]['mean'] for name in component_names]
        stats['median_per_component'] = [per_component[name]['median'] for name in component_names]
        stats['var_per_component'] = [per_component[name]['var'] for name in component_names]
        stats['std_per_component'] = [per_component[name]['std'] for name in component_names]

        return stats

    def _create_empty_stats(self, component_names: list, subset_name: str) -> Dict[str, Any]:
    ...
    def print_manifold_evaluation_report(self, metrics: Dict[str, Any]) -> None:
        ...
        print(f"  CERTAIN (predicted in attractor):   {metrics['num_certain']} ({metrics['certain_fraction']*100:.1f}%)")
        print(f"  UNCERTAIN (predicted not in attractor): {metrics['num_uncertain']} ({metrics['uncertain_fraction']*100:.1f}%)")
        ...
        print("-"*80)
        print(f"{'Component':<20} {'FULL Mean':>12} {'CERTAIN Mean':>14} {'UNCERTAIN Mean':>16}")
        print("-"*80)
        ...
        for name in component_names:
            ...
            print(f"{name:<20} {full_mae:>12.6f} {certain_mae:>14.6f} {uncertain_mae:>16.6f}")

        print("-"*80)
        print()

        # Detailed statistics for each subset
        for subset_name, subset_stats in [('FULL', full_stats),
                                           ('CERTAIN', certain_stats),
                                           ('UNCERTAIN', uncertain_stats)]:
            print(f"\n{'='*40}")
            ...
            print(f"{'Component':<20} {'Mean':>10} {'Median':>10} {'Std':>10} {'Var':>12}")
            print("-"*62)

            for name in component_names:
                comp = subset_stats['per_component'][name]
                print(f"{name:<20} {comp['mean']:>10.6f} {comp['median']:>10.6f} "
        ...
        print("-"*70)
        print(f"{'FULL':<20} {agg['aggregated_mean']:>12.6f} {agg['aggregated_median']:>12.6f} "
              f"{agg['aggregated_std']:>12.6f} {agg['aggregated_var']:>14.8f}")
        print(f"{'CERTAIN':<20} {agg['aggregated_certain_mean']:>12.6f} {agg['aggregated_certain_median']:>12.6f} "
              f"{agg['aggregated_certain_std']:>12.6f} {agg['aggregated_certain_var']:>14.8f}")
        print(f"{'UNCERTAIN':<20} {agg['aggregated_uncertain_mean']:>12.6f} {agg['aggregated_uncertain_median']:>12.6f} "
        ...
        if 'euclidean' in metrics:
            ...
            print("="*80)
            print(f"{'Group':<25} {'FULL Mean':>12} {'CERTAIN Mean':>14} {'UNCERTAIN Mean':>16}")
            print("-"*70)
            ...
            for group_name in euc['group_names']:
                ...
                print(f"{group_name:<25} {full_mean:>12.6f} {certain_mean:>14.6f} {uncertain_mean:>16.6f}")
            ...
            for subset_name in ['full', 'certain', 'uncertain']:
                ...
                for group_name in euc['group_names']:
                    ...
                    print(f"    {group_name}: mean={stats['mean']:.6f}, median={stats['median']:.6f}, "
        ...
        if 'rmse' in metrics:
            ...
            print(f"{'Subset':<20} {'Mean':>12} {'Median':>12} {'Std':>12} {'Var':>14}")
            print("-"*70)
            print(f"{'FULL':<20} {rmse['full_mean']:>12.6f} {rmse['full_median']:>12.6f} "
                  f"{rmse['full_std']:>12.6f} {rmse['full_var']:>14.8f}")
            print(f"{'CERTAIN':<20} {rmse['certain_mean']:>12.6f} {rmse['certain_median']:>12.6f} "
                  f"{rmse['certain_std']:>12.6f} {rmse['certain_var']:>14.8f}")
            print(f"{'UNCERTAIN':<20} {rmse['uncertain_mean']:>12.6f} {rmse['uncertain_median']:>12.6f} "
