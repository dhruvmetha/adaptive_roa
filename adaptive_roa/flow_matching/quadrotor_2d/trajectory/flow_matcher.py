"""
Quadrotor 2D Trajectory Flow Matcher.

Manifold: R^2 x S^1 x R^3 (position x pitch angle x velocities)
State: [x, z, theta, x_dot, z_dot, theta_dot] (6D)
Embedding: [x, z, sin(theta), cos(theta), x_dot, z_dot, theta_dot] (7D)
Output: velocity in tangent space (6D)
"""
...
class Quadrotor2DTrajectoryFlowMatcher(TrajectoryFlowMatcherBase):
    ...
    def __init__(self, system, model, optimizer, scheduler,
        ...
        print(f"Initialized Quadrotor2DTrajectoryFlowMatcher: seq_len={sequence_length}, history={history_length}")
    ...
    def sample_noisy_input(self, batch_size, device):
        noisy = torch.randn(batch_size, 6, device=device)
    ...
    def predict_endpoint(self, start_states, num_steps=100, latent=None, method="euler"):
        ...
        if not self.use_manifold:
            ...
            result[:, 2] = torch.atan2(torch.sin(endpoints[:, 2]), torch.cos(endpoints[:, 2]))
            endpoints = result
        return endpoints
    ...
    def load_from_checkpoint(cls, checkpoint_path, device=None):
        ...
        if system is None:
            system = Quadrotor2DSystem(dataset_dir=_hp("system_dataset_dir"))
