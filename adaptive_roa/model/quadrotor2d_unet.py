...
class Quadrotor2DUNet(nn.Module):
    """
    Quadrotor 2D UNet for Latent Conditional Flow Matching:
    - Takes embedded state x_t (x_norm, z_norm, sin θ, cos θ, ẋ_norm, ż_norm, θ̇_norm), time t, latent z, condition
    - Predicts velocity in ℝ² × S¹ × ℝ³ tangent space: (dx/dt, dz/dt, dθ/dt, dẋ/dt, dż/dt, dθ̇/dt)

    Input: 7D embedded state
    Output: 6D tangent velocity
    """
    def __init__(self,
                 embedded_dim: int = 7,        # Quadrotor2D embedded state dimension
                 latent_dim: int = 0,          # Latent variable dimension
                 condition_dim: int = 7,       # Condition dimension (embedded start state)
                 time_emb_dim: int = 64,       # Time embedding dimension
                 hidden_dims = [256, 512, 256], # Hidden layer dimensions
                 output_dim: int = 6,          # Output dimension (6D velocity)
    ...
    def forward(self,
                x_t: torch.Tensor,        # [B, 7] embedded state
                t: torch.Tensor,          # [B] time
                z: torch.Tensor,          # [B, latent_dim] latent vector
                condition: torch.Tensor   # [B, 7] embedded start state
                ) -> torch.Tensor:
        """
        Forward pass for Quadrotor 2D Latent Conditional Flow Matching

        Args:
            x_t: Current embedded state [B, 7]
            t: Time [B]
            z: Latent variable [B, latent_dim]
            condition: Start state condition [B, 7]

        Returns:
            Predicted velocity [B, 6] in tangent space
        """
        # Time embedding
        t_emb = timestep_embedding(t, self.time_emb_dim)  # [B, time_emb_dim]

        if self.use_input_embeddings:
            state_emb = self.state_embedding(x_t)
            latent_emb = self.latent_embedding(z)
            condition_emb = self.condition_embedding(condition)
            x_input = torch.cat([state_emb, t_emb, latent_emb, condition_emb], dim=1)
        else:
            x_input = torch.cat([x_t, t_emb, z, condition], dim=1)

        # Predict velocity in tangent space
        velocity = self.vel_head(x_input)  # [B, 6]
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            "model_type": "Quadrotor 2D Latent Conditional Flow Matching"
        }
