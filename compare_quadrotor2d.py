#!/usr/bin/env python3
"""Compare evaluation metrics across adaptive sampling strategies for Quadrotor2D (joint optimization only)."""
...
OUTPUT_PATH = "/common/home/dm1487/robotics_research/tripods/olympics-classifier/quadrotor2d_comparison.pdf"
...
def main():
    ...
    for run in RUNS:
        ...
        d = load_run_data(run["base"], EVAL_SUBDIR, EPOCHS)
        run_data.append(d)
    ...
    fig.suptitle("Quadrotor2D: Sampling Strategy Comparison (Joint Optimization)", fontsize=16, fontweight="bold", y=0.99)
    ...
    print("=" * 120)
    print(f"{'Method':<25} {'Trajs':<7} {'F1':<8} {'Sep%':<9}")
    print("-" * 120)
    for run, d in zip(RUNS, run_data):
        print(f"{run['label']:<25} {d['train_trajectories'][-1]:<7} {d['qhat_f1'][-1]:<8.4f} {d['qhat_sep'][-1]:<9.2f}")
