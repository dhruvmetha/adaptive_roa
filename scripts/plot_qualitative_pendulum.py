...
CACHE_FILE = Path("qualitative_pendulum_cache.npz")
...
def compute_all_grids(device: str, epochs: list[int] = None):
    ...
    for method_name, run_dir in RUNS.items():
        ...
        for epoch in epochs:
            ...
            pred_labels, p_success, p_failure, p_invalid = classify_grid(
                model, model.system, grid_np, lam, delta, device
            )
...
def plot_figure(results, theta, theta_dot, epochs: list[int] = None,
    ...
    # Save
    for ext in ["pdf", "png"]:
        out = f"qualitative_pendulum.{ext}"
...
def main():
    ...
    if CACHE_FILE.exists() and not args.recompute:
        ...
        if missing:
            ...
            new_results, theta, theta_dot = compute_all_grids(args.device, missing)
            for m in new_results:
                results.setdefault(m, {}).update(new_results[m])
            save_cache(results, theta, theta_dot)
    else:
        results, theta, theta_dot = compute_all_grids(args.device, compute_epochs)
        save_cache(results, theta, theta_dot)
