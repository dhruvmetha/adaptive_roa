#!/usr/bin/env python3
"""
Re-evaluate Quadrotor2D models from all epochs with configurable parameters.
...
Usage:
    ...
    python scripts/reevaluate_quadrotor2d.py /path/to/training/output
    ...
    # Custom output dir (overrides auto-naming)
    python scripts/reevaluate_quadrotor2d.py /path/to/training/output \
        --attractor_radius 0.25 \
...
from run_adaptive_quadrotor2d import evaluate_full_roa_fast
...
def load_checkpoint(epoch_dir: Path, device: str):
    ...
    return Quadrotor2DLatentConditionalFlowMatcher.load_from_checkpoint(
...
def main():
    parser = argparse.ArgumentParser(
        description="Re-evaluate Quadrotor2D models with configurable parameters"
    )
    parser.add_argument(
        "training_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum epoch to evaluate (evaluates all epochs <= max_epoch)",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    # RE-EVALUATION MODE
    print("=" * 70)
    print("RE-EVALUATION MODE - Quadrotor2D")
    print("=" * 70)
