#!/usr/bin/env python3
"""
Re-evaluate Quadrotor3D models from all epochs with configurable parameters.
...
Usage:
    ...
    python scripts/reevaluate_quadrotor3d.py /path/to/training/output
...
from run_adaptive_quadrotor3d import evaluate_full_roa_fast
...
def load_checkpoint(epoch_dir: Path, device: str):
    ...
    return Quadrotor3DLatentConditionalFlowMatcher.load_from_checkpoint(
...
def recompute_qhat(
    ...
    # Quadrotor3D uses two_sided decision rule
...
def main():
    parser = argparse.ArgumentParser(
        description="Re-evaluate Quadrotor3D models with configurable parameters"
    )
    parser.add_argument(
        "training_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum epoch number to evaluate (evaluates all epochs up to and including this)",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    # RE-EVALUATION MODE
    print("=" * 70)
    print("RE-EVALUATION MODE - Quadrotor3D")
    print("=" * 70)
    ...
    if args.epoch is not None:
        ...
        if args.max_epoch is not None:
            ...
            if not epoch_dirs:
                print(f"ERROR: No epochs found <= {args.max_epoch} in {training_dir}")
                return 1
        ...
        max_info = f" (up to epoch {args.max_epoch})" if args.max_epoch is not None else ""
