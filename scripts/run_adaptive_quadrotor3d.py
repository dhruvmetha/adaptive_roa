"""
Run Adaptive Sampling Pipeline for Quadrotor 3D.

This script demonstrates the full adaptive sampling loop for the 3D quadrotor system:
...
Usage:
    python scripts/run_adaptive_quadrotor3d.py
    python scripts/run_adaptive_quadrotor3d.py --config-name=adaptive_quadrotor3d
"""
import os
import hydra
...
from adaptive_roa.utils.env_config import get_net_id, get_exp_dir, get_data_dir, get_env_config, get_shared_data_base
...
def plot_quadrotor3d_roa_projections(
    ...
    delta: float,
    output_file: str,
    title: str = "Quadrotor 3D ROA Phase Space Projections",
):
    """
    Plot the Region of Attraction (ROA) for Quadrotor 3D as 2D projections.

    Since Quadrotor 3D has 13D state (x, y, z, qw, qx, qy, qz, x_dot, y_dot, z_dot, p, q, r),
    we create multiple 2D projections:
    - (x, y): Horizontal position
    - (x, z): Vertical position
    - (x_dot, y_dot): Horizontal velocity
    - (x_dot, z_dot): Vertical velocity
    - (p, q): Angular velocity (roll/pitch rates)
    - (qw, ||q_vec||): Quaternion scalar vs vector magnitude

    Args:
        start_states: [N, 13] array of quadrotor states
        success_rate: [N] array of p(success) from MC sampling
        true_labels: [N] array of ground truth labels (1=success, -1=failure, 0=separatrix)
    ...
    # State ordering: (x, y, z, qw, qx, qy, qz, x_dot, y_dot, z_dot, p, q, r)
    ...
    # Compute quaternion vector magnitude for visualization
    q_vec_mag = np.sqrt(start_states[:, 4]**2 + start_states[:, 5]**2 + start_states[:, 6]**2)

    # Define projections: (x_idx, y_idx, x_label, y_label, x_lim, y_lim)
    projections = [
        (0, 1, 'x (m)', 'y (m)', None, None),
        (0, 2, 'x (m)', 'z (m)', None, None),
        (7, 8, r'$\dot{x}$ (m/s)', r'$\dot{y}$ (m/s)', None, None),
        (7, 9, r'$\dot{x}$ (m/s)', r'$\dot{z}$ (m/s)', None, None),
        (10, 11, 'p (rad/s)', 'q (rad/s)', None, None),
        ('qw', 'qvec', 'qw', r'$||q_{vec}||$', (0, 1), (0, 1)),  # Special case
    ]

    for col, proj in enumerate(projections):
        if proj[0] == 'qw':  # Special quaternion projection
            xi_pred = start_states[pred_order, 3]  # qw
            yi_pred = q_vec_mag[pred_order]
            xi_gt = start_states[gt_order, 3]  # qw
            yi_gt = q_vec_mag[gt_order]
            xlabel, ylabel = proj[2], proj[3]
            xlim, ylim = proj[4], proj[5]
        else:
            xi, yi = proj[0], proj[1]
            xi_pred = start_states[pred_order, xi]
            yi_pred = start_states[pred_order, yi]
            xi_gt = start_states[gt_order, xi]
            yi_gt = start_states[gt_order, yi]
            xlabel, ylabel = proj[2], proj[3]
            xlim, ylim = proj[4], proj[5]
    ...
    print(f"Saved Quadrotor 3D ROA projections to: {output_file}")
    ...
    return {
        ...
        'n_gt_separatrix': int(n_gt_sep),
    }


def plot_quadrotor3d_probability_heatmap(
    start_states: np.ndarray,
    success_rate: np.ndarray,
    lambda_star: float,
    delta: float,
    output_file: str,
    title: str = "Quadrotor 3D ROA - Success Probability",
):
    """
    ...
    for col, proj in enumerate(projections):
        ...
        if proj[0] == 'qw':  # Special quaternion projection
            x_data = start_states[:, 3]  # qw
            y_data = q_vec_mag
            xlabel, ylabel = proj[2], proj[3]
            xlim, ylim = proj[4], proj[5]
        else:
            xi, yi = proj[0], proj[1]
            x_data = start_states[:, xi]
            y_data = start_states[:, yi]
            xlabel, ylabel = proj[2], proj[3]
            xlim, ylim = proj[4], proj[5]
    ...
    print(f"Saved Quadrotor 3D probability heatmap to: {output_file}")
...
def compute_metrics_notebook_style(p_success: np.ndarray, p_failure: np.ndarray,
    ...
    This properly handles Quadrotor 3D's three-way classification where:
...
def compute_metrics_lambda_delta_quadrotor3d(
...
def evaluate_full_roa_fast(
    ...
    if verbose:
        print(f"\n{'='*60}")
        print("FULL ROA EVALUATION (fast batched) - Quadrotor 3D")
        print(f"{'='*60}")
    ...
    # 1. lambda*+-delta from conformal prediction (uses BOTH p_success and p_failure)
    metrics_conformal = compute_metrics_lambda_delta_quadrotor3d(
        p_success=p_success,
        p_failure=p_failure,
        y_all=y_all,
    ...
    if output_file:
        ...
        np.savez(
            npz_file,
            start_states=X_all,           # [N, 13] - Quadrotor 3D state
        ...
        plot_quadrotor3d_roa_projections(
            start_states=X_all,
            ...
            title="Quadrotor 3D ROA - Epoch Evaluation"
        )

        # Generate probability heatmap
        heatmap_file = output_file.replace('.json', '_roa_heatmap.png')
        plot_quadrotor3d_probability_heatmap(
            start_states=X_all,
            success_rate=p_success,
            lambda_star=lambda_star,
            delta=delta,
            output_file=heatmap_file,
            title="Quadrotor 3D ROA - Success Probability"
        )
...
def train_flow_matcher(
    ...
    data_module = Quadrotor3DEndpointDataModule(
    ...
    if cfg.model.get('output_dim', 12) != model_output_dim:
        print(f"   Auto-adjusting model.output_dim: {cfg.model.get('output_dim', 12)} → {model_output_dim} (use_manifold={use_manifold})")
        with open_dict(cfg):
            cfg.model.output_dim = model_output_dim
...
@hydra.main(config_path="../configs", config_name="adaptive_quadrotor3d", version_base=None)
def main(cfg: DictConfig):
    """Main adaptive sampling loop."""
    print("=" * 70)
    print("ADAPTIVE SAMPLING PIPELINE - Quadrotor 3D")
    print("=" * 70)
    ...
    conformal_config = ConformalConfig(
        ...
        attractor_radius=cfg.conformal.get('attractor_radius', 0.05),
        ...
        decision_rule=cfg.conformal.get('decision_rule', 'two_sided'),  # Default two_sided for 3-way classification
    ...
    for epoch in range(n_epochs):
        ...
        # 8a: Load held-out calibration set and calibrate NEW q_hat
        ...
        full_roa_metrics = evaluate_full_roa_fast(
            flow_matcher=flow_matcher,
            system=system,
            eval_states_file=cfg.data_source.test_set_file,
        ...
        # Store both q_hats in results
        full_roa_metrics['q_hat_training'] = float(q_hat)
        full_roa_metrics['q_hat_eval'] = float(q_hat_eval)
        full_roa_metrics['n_cal_eval'] = len(X_cal_eval)
        ...
        epoch_result = {
            ...
            'q_hat': float(q_hat),              # Training-time q_hat (used for sampling)
            'q_hat_eval': float(q_hat_eval),    # Eval-time q_hat (for proper guarantees)
            'n_cal_eval': len(X_cal_eval),      # Held-out calibration set size
            'optimize_mode': cfg.conformal.get('optimize_mode', 'lambda'),
