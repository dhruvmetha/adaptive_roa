...
- ``report``: aggregate whatever ``collect_runs`` finds under ``--exp-root``
  ...
  unmatched budget, an incomparable metric, or seeds that never actually
  diverged. A guard raising here is the harness working, not a bug in this
  script -- see ``report.py``'s module docstring.
"""
...
from adaptive_roa.benchmark.guards import assert_all_complete
...
from adaptive_roa.benchmark.report import METRIC, build_report
...
def _run_report(args) -> None:
    ...
    text = build_report(df, metric=args.metric)
