"""
CartPole All-Points Classification Data Module
...
State space (4D input, 5D output):
- Position: x (normalized to [-1, 1])
- Orientation: theta → sin(theta), cos(theta) (S¹ manifold embedding)
- Velocity: x_dot (normalized to [-1, 1])
- Angular velocity: theta_dot (normalized to [-1, 1])

Input dimension: 4D raw state
Output dimension: 5D embedded state (with sin/cos for theta)
"""
...
class CartPoleAllPointsDataset(Dataset):
    """Dataset for CartPole classification from pre-built all-points files."""
    ...
    def __init__(self, data: List[str]):
        """
        Args:
            data: List of data lines (space-separated: 4 state values + 1 label)
        """
        self.data = data
    ...
    def _embed_state(self, state: np.ndarray) -> np.ndarray:
        ...
        # Embed theta on S¹ manifold using sin/cos
...
class CartPoleAllPointsDataModule(pl.LightningDataModule):
    """Lightning DataModule for CartPole all-points classification."""
    ...
    def prepare_data(self):
        ...
        print("Loading CartPole all-points classification data...")
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            self.train_dataset = CartPoleAllPointsDataset(data=self.train_data)
            ...
            self.val_dataset = CartPoleAllPointsDataset(data=self.val_data)

        if (stage == "test" or stage is None) and self.test_file:
            self.test_dataset = CartPoleAllPointsDataset(data=self.test_data)
