"""
Enhanced ROA Classification Evaluation - Excluding Separatrix Points

This enhanced version identifies separatrix points (those that don't converge to any attractor)
and excludes them from all statistical calculations to focus on well-defined basin regions.
...
def create_separatrix_analysis_plots(results, output_dir):
    ...
    ax2.set_title('Three-Class Classification Results')
    ax2.legend()
    ...
    ax3.set_title(f'Target Attractor Probabilities\n(Threshold: {params["prob_threshold"]})')
    ...
    if results['basin_only'] is not None:
        ...
        ax4.set_title('Performance: Full Dataset vs Basin Only')
        ax4.set_xticks(x)
    ...
    ax5.set_title('Target Probability by Class')
    ax5.legend()
    ...
    for i, (label, value) in enumerate(stats_data):
        ax6.text(0.05, 0.9 - i*0.15, f"{label}:", fontweight='bold', transform=ax6.transAxes)
        ax6.text(0.55, 0.9 - i*0.15, value, transform=ax6.transAxes)
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'separatrix_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print(f"🎨 Separatrix analysis visualization saved: separatrix_analysis.png")
...
def save_enhanced_results(results, output_dir):
    ...
    with open(output_dir / 'roa_classification_report_no_separatrix.txt', 'w') as f:
        f.write("ENHANCED ROA CLASSIFICATION EVALUATION REPORT\n")
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced ROA Classification - Excludes Separatrix Points")
    ...
    parser.add_argument("--output_dir", type=str, default="roa_classification_no_separatrix", help="Output directory")
