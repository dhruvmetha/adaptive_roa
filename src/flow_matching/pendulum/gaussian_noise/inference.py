"""
Inference wrapper for Pendulum Gaussian Noise Flow Matching
...
from src.flow_matching.pendulum.gaussian_noise.flow_matcher import PendulumGaussianNoiseFlowMatcher


class PendulumGaussianNoiseInference:
    """
    Inference wrapper for Gaussian Noise Flow Matching
    ...
    Usage:
        ...
        inferencer = PendulumGaussianNoiseInference(
            "outputs/pendulum_gaussian_noise_fm/2025-10-21_12-30-45"
        )

        # Single prediction
        start_states = torch.tensor([[0.5, 1.0]])  # [θ, θ̇]
    ...
    def __init__(self, checkpoint_path: Union[str, Path], device: Optional[str] = None):
        ...
        self.model = PendulumGaussianNoiseFlowMatcher.load_from_checkpoint(
            checkpoint_path,
            device=self.device
        )
    ...
    def predict_endpoint(self,
        ...
        Args:
            start_states: Start states [B, 2] as (θ, θ̇) in raw coordinates
    ...
    def compute_uncertainty(self,
        ...
        Returns:
            Dictionary with uncertainty metrics:
                - mean: Mean predicted endpoint [B, 2]
                - std: Standard deviation [B, 2]
                - samples: All samples [B, num_samples, 2]
        """
        ...
        # Compute statistics
        mean = endpoints_reshaped.mean(dim=1)  # [B, 2]
        std = endpoints_reshaped.std(dim=1)    # [B, 2]
    ...
    def __repr__(self):
        return (f"PendulumGaussianNoiseInference("
