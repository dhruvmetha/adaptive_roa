...
class CartPoleUNetFiLM(nn.Module):
    """
    CartPole UNet variant with Residual FiLM conditioning.

    Inputs:
      - embedded cartpole state x_t (5D: x_norm, x_dot_norm, sin θ, cos θ, θ̇_norm)
      - time t
      - condition (5D embedded start state)
    Output:
      - Predicts velocity in ℝ² × S¹ × ℝ tangent space (4D)
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x_t: Current embedded state [B, 5] (x_norm, x_dot_norm, sin θ, cos θ, θ̇_norm)
            t: Time [B]
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            "model_type": "CartPole Residual FiLM",
        }
