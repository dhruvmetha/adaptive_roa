"""Mountain Car UNet with FiLM Conditioning

Feature-wise Linear Modulation (FiLM) for better conditioning on start state.
Based on HumanoidUNetFiLM architecture.
"""
...
class MountainCarUNetFiLM(nn.Module):
    """
    Mountain Car UNet variant with Residual FiLM conditioning.

    Inputs:
      - embedded state x_t (2D: position, velocity)
      - time t
      - condition (2D embedded start state)
    Output:
      - Predicts velocity in ℝ² (2D: d_position, d_velocity)
    ...
    def get_model_info(self) -> dict:
        total_params = sum(p.numel() for p in self.parameters())
        return {
            "model_name": "MountainCarUNetFiLM",
            ...
            "model_type": "MountainCar Residual FiLM",
        }
