...
class PendulumCartesianUNet(nn.Module):
    """
    Pendulum Cartesian UNet for Latent Conditional Flow Matching:
    - Takes Pendulum Cartesian state x_t (x, y, vx, vy), time t, latent z, condition
    - Predicts velocity in ℝ⁴ tangent space: (dx/dt, dy/dt, dvx/dt, dvy/dt)

    Pure Euclidean manifold (ℝ⁴):
    - Input dimension: 4D (x, y, vx, vy)
    - Output dimension: 4D (dx/dt, dy/dt, dvx/dt, dvy/dt)
    - Condition dimension: 4D (same as embedded state)
    - No embedding transformation needed (unlike CartPole with SO2 angle)
    """
    def __init__(self,
                 embedded_dim: int = 4,        # Pendulum Cartesian state dimension (pure Euclidean)
    ...
    def forward(self,
                x_t: torch.Tensor,        # [B, 4] embedded state (x, y, vx, vy)
                t: torch.Tensor,          # [B] time
                z: torch.Tensor,          # [B, latent_dim] latent vector
                condition: torch.Tensor   # [B, 4] embedded start state
                ) -> torch.Tensor:
        """
        Forward pass for Pendulum Cartesian Latent Conditional Flow Matching
        ...
        Returns:
            Predicted velocity [B, 4] in tangent space (dx/dt, dy/dt, dvx/dt, dvy/dt)
        """
        # Time embedding
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            "model_type": "Pendulum Cartesian Latent Conditional Flow Matching"
        }
