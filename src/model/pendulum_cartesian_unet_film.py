...
class PendulumCartesianUNetFiLM(nn.Module):
    """
    Pendulum Cartesian UNet variant with Residual FiLM conditioning.

    Inputs:
      - embedded pendulum state x_t (4D: x, y, ẋ, ẏ)
      - time t
      - condition (4D embedded start state)
    Output:
      - Predicts velocity in ℝ⁴ tangent space (4D: dx/dt, dy/dt, dẋ/dt, dẏ/dt)
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x_t: Current embedded state [B, 4] (x, y, ẋ, ẏ)
            t: Time [B]
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            "model_type": "Pendulum Cartesian Residual FiLM",
        }
