...
class PendulumUNetFiLM(nn.Module):
    """
    Pendulum UNet variant with Residual FiLM conditioning.

    Inputs:
      - embedded pendulum state x_t (3D: sin θ, cos θ, θ̇_norm)
      - time t
      - condition (3D embedded start state)
    Output:
      - Predicts velocity in S¹ × ℝ tangent space (2D: dθ/dt, dθ̇/dt)
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x_t: Current embedded state [B, 3] (sin θ, cos θ, θ̇_norm)
            t: Time [B]
            condition: Start state condition [B, 3]
    ...
    def get_model_info(self) -> dict:
        ...
        return {
            ...
            "model_type": "Pendulum Residual FiLM",
        }
