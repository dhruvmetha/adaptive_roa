...
# Takes the parent's actual device IDS, not a count: writing back an absolute
# index discards the parent's restriction and lands every member on physical
# device 0 wherever cgroup isolation is absent (see test_member_device_placement). Amarel gpu-redhat nodes top out
...
def test_device_for_member_one_gpu_per_member_when_gpus_cover_members():
    ...
    assert [_device_for_member(r, ids) for r in range(5)] == ["0", "1", "2", "3", "4"]
...
def test_device_for_member_wraps_when_fewer_gpus_than_members():
    ...
    assert [_device_for_member(r, ids) for r in range(5)] == ["0", "1", "2", "3", "0"]
...
def test_device_for_member_wraps_repeatedly_with_very_few_gpus():
    ...
    assert [_device_for_member(r, ids) for r in range(5)] == ["0", "1", "0", "1", "0"]
...
def test_device_for_member_distribution_is_balanced():
    ...
    for n_members, n_visible in cases:
        ...
        assignments = [_device_for_member(r, ids) for r in range(n_members)]
